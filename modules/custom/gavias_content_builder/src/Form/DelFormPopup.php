...
class DelFormPopup extends FormBase{
  ...
  protected function dialog($values = array()){
    ...
    $response->addCommand(new InvokeCommand(".field--type-gavias-content-builder .gva-choose-gbb.gva-id-{$random} .gbb-item.disable", 'addClass', array('active')));

    $response->addCommand(new InvokeCommand('.field_gavias_content_builder.gva-id-'.$random, 'val', array('')));
