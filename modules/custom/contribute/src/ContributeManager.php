...
/**
 * Class contribute manager.
 */
class ContributeManager implements ContributeManagerInterface {
  ...
  public function __construct(CacheBackendInterface $cache, ClientInterface $http_client, DateFormatterInterface $date_formatter, ConfigFactoryInterface $config_factory) {
    ...
    $this->status = $this->configFactory->get('contribute.settings')->get('status');
    $this->accountType = $this->configFactory->get('contribute.settings')->get('account_type');
    $this->accountId = $this->configFactory->get('contribute.settings')->get('account_id');
  }
  ...
  public function getAccount($display_type = TRUE) {
    ...
    $this->cache->set($cid, $account, strtotime('+1 hour'), ['contribute']);
  ...
  public function getMembership() {
    ...
    $this->cache->set($cid, $membership, strtotime('+1 hour'), ['contribute']);
